...
read(Key, [#data{key=Key2, data=Element2}|Tdb]) ->
	case Key2 of
		Key	-> {ok, Element2};
		Other		-> read(Key, Tdb)
	end.
...
delete(Key, [Hdb|Tdb], NewDb) ->
	case Hdb of
		[]			-> [];
		#data{key=Key, data=_}	-> delete(Key, Tdb, NewDb);
...
match(Element, [#data{key=Key, data=Elm}|Tdb], []) ->
	case Elm of
		Element	-> match(Element, Tdb, [Key]);
		Other	-> match(Element, Tdb, [])
	end;

match(Element, [#data{key=Key, data=Elm}|Tdb], Accum) ->
	case Elm of
		Element	-> match(Element, Tdb, Accum++[Key]);
